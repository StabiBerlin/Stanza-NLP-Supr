    ...
    while True:
        ...
        if not line:
            ...
            if len([word for word in ud.words[sentence_start:] if word.parent is None]) != 1:
                print("WARNUNG: Mehrere ROOT-Knoten in einem Satz gefunden, aber fortgesetzt.")
                # raise UDError("There are multiple roots in a sentence")  # Deaktiviert
...
def evaluate(gold_ud, system_ud, skip_token_check=False):
    class Score:
    ...
    if gold_ud.characters != system_ud.characters and not skip_token_check:
        raise UDError(
    ...
    elif gold_ud.characters != system_ud.characters:
        print(
            "WARNUNG: Die Tokens in den Gold- und Systemdateien unterscheiden sich!"
        )
...
def evaluate_wrapper(args):
    ...
    return evaluate(gold_ud, system_ud, args.skip_token_check)

def main():
    # Parse arguments
    parser = argparse.ArgumentParser()
    parser.add_argument("gold_file", type=str,
                        help="Name der CoNLL-U Datei mit den Gold-Annotationsdaten.")
    parser.add_argument("system_file", type=str,
                        help="Name der CoNLL-U Datei mit den vorhergesagten Daten.")
    parser.add_argument("--verbose", "-v", default=False, action="store_true",
                        help="Drucke alle Metriken aus.")
    parser.add_argument("--counts", "-c", default=False, action="store_true",
                        help="Drucke rohe Zählwerte anstelle von Präzision/Recall/F1.")
    parser.add_argument("--skip-token-check", default=False, action="store_true",
                        help="Deaktiviere die Überprüfung der Token-Konkatenation.")
